layui.use(['table','common','jquery'], function(){
    ...
    $(document).on('click','#initglobal',function(){
    	 layer.confirm('初始化global会把现有的参数覆盖掉是否确认初始化?', {
    	        ...
    	        layer.close();
    	        common.ajax('/mysql/initGlobal/manage', 'post', 'json', {}, function (res) {
    	        	if(res.code=="0"){
    	        		//同步更新表格和缓存对应的值
    ...
    $(document).on('click','#initOther',function(){
        layer.confirm('初始化用户管理数据部署项目时只需要初始化一次,是否确认初始化?', {
            btn: ['确定','取消'] //按钮
        }, function(index){
            layer.close();
            common.ajax('/Imysql/init', 'post', 'json', {}, function (res) {
                if(res.code=="0"){
                    //同步更新表格和缓存对应的值
    ...
    table.on('tool(test-table-demoEvent)', function(obj){
      ...
      if(obj.event === 'setSign'){
        layer.prompt({
          ...
          layer.close(index);
          //这里一般是发送修改的Ajax请求
			common.ajax('/mysql/updateManage', 'post', 'json', {
            	'key': data.key,
            	'value': value
        	}, function (res) {
	        	if(res.code=="0"){
