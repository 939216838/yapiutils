layui.use(['table','common','jquery'], function(){
    ...
    $(document).on('click','#initglobal',function(){
    	 layer.confirm('初始化global会把现有的参数覆盖掉是否确认初始化?', {
    	        ...
    	        layer.close();
    	        common.ajax('/ff/initGlobal', 'post', 'json', {}, function (res) {
    	        	if(res.code=="0"){
    	        		//同步更新表格和缓存对应的值
    ...
    //监听初始化渠道按钮
    $(document).on('click','#initChannel',function(){
    	layer.confirm('初始化渠道会把现有的参数覆盖掉是否确认初始化?', {
    		btn: ['确定','取消'] //按钮
    	}, function(index){
    		layer.close();
    		common.ajax('/ff/initChannel', 'post', 'json', {}, function (res) {
    			if(res.code=="0"){
    				//同步更新表格和缓存对应的值
    				layer.alert(res.data);
    				table.reload("test-table-demoEvent", { //此处是上文提到的 初始化标识id
    ...
    table.on('tool(test-table-demoEvent)', function(obj){
      ...
      if(obj.event === 'setSign'){
        layer.prompt({
          ...
          layer.close(index);
          //这里一般是发送修改的Ajax请求
			common.ajax('/ff/update', 'post', 'json', {
            	'key': data.key,
            	'value': value
        	}, function (res) {
	        	if(res.code=="0"){
